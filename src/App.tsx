...
function App({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const generateIncomeCertificate = async () => {
    try {
      ...
      for (let i = 0; i < 3; i++) {
        const date = new Date(now.getFullYear(), now.getMonth() - i, 1);
        const monthName = date.toLocaleString('es-MX', { month: 'long', year: 'numeric' }).toUpperCase();
        const monthKey = `${meses[date.getMonth()]}_${date.getFullYear()}`;
        months.push(monthKey);
      }

      const monthsData: Array<{month: string, days: Array<{dia: number, ganancia: number}>, total: number}> = [];
      ...
      const parrafo1 = `SpeedCabsZapotlan S.A.S de C.V., a través de su Director General, Ernesto Ruiz Arellano, hace constar lo siguiente:`;
      ...
      const parrafo2 = `${conductorNombre}, con correo electrónico ${user.email}${userData?.telefono ? ' y teléfono ' + userData.telefono : ''}, trabaja actualmente en esta compañía ocupando el puesto de Conductor de Taxi. ${conductorNombre} ha prestado sus servicios a esta empresa.`;
      ...
      pdfDoc.setFont(undefined, 'bold');
      pdfDoc.text('Ernesto Ruiz Arellano', 20, yPosition);
      yPosition += 5;
      pdfDoc.setFont(undefined, 'normal');
